...
public final class HarvestAI_F1 {
    ...
    public static boolean tick(FermerPetsModule plugin, LivingEntity pet, Player owner){
        ...
        Debug.logPet(pet, "HarvestAI_F1.java: tick start");
            ...
            if (upkeep > 0){
                ...
                if (last == 0L){
                    ...
                    if (fuel.take(owner.getUniqueId(), upkeep)){
                        ...
                        try { plugin.getManager().unsummonAndCleanup(owner.getUniqueId(), 1); } catch (Throwable ignored2) {
        ...
        if (nowMs - lastMs < delayMs){ com.fermerpets.FlightLanding.manage(pet, com.fermerpets.TargetTracker.get(pet.getUniqueId()));
            Debug.logPet(pet, "HarvestAI_F1.java: skip due to throttle"); return true; }
        ...
        if (target == null){ Debug.logPet(pet, "HarvestAI_F1.java: no target in range"); return false; } else { Debug.logPet(pet, "HarvestAI_F1.java: target="+target.type+" at "+locStr(target.location)); }
        ...
        if (pet.getLocation().distanceSquared(target.location) > 2.25) {
            ...
            com.fermerpets.Debug.logPet(pet, "HarvestAI_F1.java: approach phase; dist2="+pet.getLocation().distanceSquared(target.location));
            return true;
        }
        ...
        if (target.type == TargetType.ITEM) {
            ...
            if (it != null && it.isValid()) {
                Debug.logPet(pet, "HarvestAI_F1.java: pickup item "+it.getItemStack()); pushLoot(pet.getUniqueId(), it.getItemStack().clone());
            ...
            if (b != null) {
                Debug.logPet(pet, "HarvestAI_F1.java: harvest block "+b.getType()); harvested = harvestBlock(pet, b);
            }
        }
        ...
        if (plugin.getConfig().getBoolean("harvest.teleport-deliver", true)) {
            Debug.logPet(pet, "HarvestAI_F1.java: performDelivery to owner "+owner.getName()); performDelivery(pet, owner, useEnderFx);
        }
        return true;
    }
    ...
    private static void rememberHarvestSpot(LivingEntity pet, Location loc){
        ...
        lastHarvestLocation.put(pet.getUniqueId(), loc.clone()); Debug.logPet(pet, "HarvestAI_F1.java: rememberHarvestSpot "+locStr(loc));
    }
    ...
    private static Target findTarget(LivingEntity pet, int range, String currentFilter){
        ...
        // Сначала подбираем предметы, чтобы лут не скапливался на земле.
    ...
    private static void moveTowards(LivingEntity pet, Location target, boolean flightParticles){
        ...
        Vector walkVel = dir.clone().multiply(Math.min(0.35, dist * 0.25)); Debug.logPet(pet, "HarvestAI_F1.java: move WALK towards "+locStr(target));
        ...
        try {
            ...
            if (front.getType().isSolid() && pet.isOnGround()) { Debug.logPet(pet, "HarvestAI_F1.java: move SPIDER-HOP obstacle="+front.getType());
        ...
        if (needFlight) { Debug.logPet(pet, "HarvestAI_F1.java: move ALLAY-FLY (gap/vertical)");
    ...
    private static void pushLoot(UUID petId, ItemStack st){
        ...
        lootBuffer.computeIfAbsent(petId, k -> new ArrayList<>()).add(st); Debug.logPet(Bukkit.getEntity(petId), "HarvestAI_F1.java: pushLoot "+st);
    }
    ...
    private static void performDelivery(LivingEntity pet, Player owner, boolean useEnderFx){
        ...
        Debug.logPet(pet, "HarvestAI_F1.java: TELEPORT to OWNER "+owner.getName()+" at "+locStr(ownerLoc)); pet.teleport(ownerLoc);
        ...
        if (useEnderFx) playEnderFx(pet.getLocation());
        Debug.logPet(pet, "HarvestAI_F1.java: TELEPORT back to LAST spot "+locStr(returnPoint)); pet.teleport(returnPoint);
